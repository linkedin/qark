    ...
    for node in common.xmldoc.getElementsByTagName('provider'):
        ...
        if 'android:grantUriPermissions' in node.attributes.keys():
            if (node.attributes['android:grantUriPermissions'].value == 'true'):
                common.logger.warning("Permission can be granted to any of the Content Provider " + str(
                    cpName) + "'s data, this is probably not intentional and could potentially lead to information disclosure. Please review AndroidManifest.xml. More details: http://developer.android.com/guide/topics/manifest/provider-element.html")
            else:
                        ...
                        if x.attributes['android:path'].value == '/':
                            if 'android:pathPrefix' not in x.attributes.keys():
                                common.logger.warning(
                                    "This application appears to allow granting access to the root path / of the ContentProvider " + str(
                                        cpName) + ", which could lead to information disclosure, due to the use of an / as the path value, with no pathPrefix specified in the grant-uri-permission element. Please review AndroidManifest.xml. More details: http://developer.android.com/guide/topics/manifest/provider-element.html")
    return
