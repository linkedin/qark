                ...
                if not re.search(r'\*', imp_decl.name.value):
                    ...
                    if os.path.isfile(classFile):
                        try:
                            ...
                            common.logger.error(
                                "Could not create tree for " + str(classFile) + " in externalMethodDeclarations.py")
                            continue
                                ...
                                if type(type_decl) is m.ClassDeclaration:
                                    for t in type_decl.body:
                                        try:
                                            ...
                                            common.logger.error(
                                                "Problem running recursive_method_finder for m.ClassDeclaration in externalMethodDeclarations.py: " + str(
                                                    e))
                                    ...
                                    for y in t:
                                        try:
                                            ...
                                            common.logger.error(
                                                "Problem running recursive_method_finder for lists in externalMethodDeclarations.py: " + str(
                                                    e))
                                    ...
                                    for f in t._fields:
                                        try:
                                            ...
                                            common.logger.error(
                                                "Problem running recursive_method_finder for _fields in externalMethodDeclarations.py: " + str(
                                                    e))
                        else:
                            common.logger.error("There was a problem reading the imported class: " + str(
                                classFile) + ". Results may be negatively impacted if the class contains security relevant methods. This may be due to a de-compilation error.")
                            continue
                    else:
                        if common.source_or_apk == 1:
                            # TODO check against *every* standard class to see if it's likely from a standard library?
                            common.logger.debug("Imported class file not found: " + str(
                                imp_decl.name.value) + ". This may be due to a de-compilation error. The results may be degraded if a significant method declaration cannot be resolved.")
                            common.parsingerrors.add(str(current_file))
                        else:
                            # TODO - allow for including source from multiple directories
                            common.parsingerrors.add(str(current_file))
                            common.logger.debug("The imported class: " + str(
                                imp_decl.name.value) + ", was not found. We are still working on integrating imports from different directories. Please either try scanning the built APK, check for updates or contribute your own code. This can be safely ignored if this class is defined in standard Java or Android libraries.")
                else:
                    common.parsingerrors.add(str(current_file))
                    common.logger.info(
                        "Your developer used a lazy import (*). So for now we'll be lazy developers and refuse to chase it down. Check for updates soon. Sorry, but they started it...")
                    common.logger.debug(
                        "WILDCARD Import Path - We have not yet implemented the code for hunting these down, sorry!")
        else:
            break
    if numMatches > 0:
        ...
        if common.source_or_apk == 1:
            common.parsingerrors.add(str(current_file))
            common.logger.debug(
                "Unable to locate the definition of the " + str(token.name) + " method from the imports in " + str(
                    current_file) + ". This will negatively impact results if this is not part of the standard Java/Android libraries and is a security relevant method.")
        else:
            common.parsingerrors.add(str(current_file))
            common.logger.debug(
                "Unable to locate the definition of the " + str(token.name) + " method from the imports in " + str(
                    current_file) + ". This will negatively impact results if this is not part of the standard Java/Android libraries and is a security relevant method. We are still working on ignoring safe standard methods. Please check back for updates or contribute your own code.")
    return
                ...
                if str(token.name) == str(t.name):
                    if len(t.parameters) == len(token.arguments):
                        try:
                            ...
                            common.logger.error(
                                "Problem during parameter type matching in externalMethodDeclaration.py's recursive_method_finder method: " + str(
                                    e))
            ...
            for i in t:
                if type(i) is m.MethodDeclaration:
                    ...
                    try:
                        ...
                        common.logger.error(
                            "Problem in recursive_method_finder, trying to iterate fields on list branch: " + str(e))
        except Exception as e:
...
def process_matches(token, tree):
    ...
    try:
        if thorough:
            ...
            common.logger.info("Only one match was found for this method: " + str(
                token.name) + ",but cannot be confirmed 100% accurate.")
            sinks_encountered(token, tree)
        else:
            common.logger.warning("A number of potentially matching method declarations were found for : " + str(
                token.name) + ". QARK cannot currently provide a 100% positive match, so false positives could arise from this.")
            sinks_encountered(token, tree)
    except Exception as e:
...
def sinks_encountered(token, tree):
    ...
    if found:
        common.logger.log(common.VULNERABILITY_LEVEL,
                          "It appears a vulnerablity was found here, but unfortunately we haven't completed this branch yet.")
    # raw_input()
    return found
