...
def show_adb_commands(component, compType, packageName):
    ...
    type_list = ['String', 'StringArray', 'StringArrayList', 'Boolean', 'BooleanArray', 'Int', 'Float', 'Long',
                 'LongArray', '[]', '', 'IntArray', 'IntegerArrayList', 'FloatArray', 'Double', 'Char', 'CharArray',
                 'CharSequence', 'CharSequenceArray', 'CharSequenceArrayList', 'Byte', 'ByteArray', 'Bundle', 'Short',
                 'ShortArray', 'Serializable', 'Parcelable', 'ParcelableArrayList', 'ParcelableArray', 'unknownType']
                                    ...
                                    if str(extras_list[t]) not in type_list:
                                        if re.match(r'^\..*', str(c[1])):
                                            baseIntent = "adb shell am start -a \"" + c[
                                                0] + "\" -n \"" + packageName + "/" + packageName + str(c[1]) + "\""
                                        else:
                                            baseIntent = "adb shell am start -a \"" + c[
                                                0] + "\" -n \"" + packageName + "/" + str(c[1]) + "\""
                                        simple_type = True
                                        ...
                                        if simple_type:
                                            ...
                                            print baseIntent + "\"" + str(
                                                extras_list[t]) + "\" \"Insert" + extra_type + "Here\""
                                            output = True
                                        ...
                                        # This is excluding all the known types, because every second element is type
                                            ...
                                            common.logger.info("Extra: " + str(extras_list[
                                                                                   t]) + " is not a simple type, or could not be determined. You'll need to append the parameter which corresponds with the correct data type, followed by a key and value, both in quotes.")
                                            ...
                                            print "\n"
                                            output = True
                                        report.write_adb_commands("adbcommands-issues-list",
                                                                  common.Severity.VULNERABILITY, baseIntent, None,
                                                                  "activity")
                                extras_list = []
                            if not output:
                                if re.match(r'^\..*', str(c[1])):
                        ...
                        for t in range(1, (len(extras_list) - 1)):
                            if str(extras_list[t]) not in type_list:
                                ...
                                # This is excluding all the known types, because every second element is type
                                    ...
                                    print "\n"
                                    ouput = True
                                report.write_adb_commands("adbcommands-issues-list", common.Severity.VULNERABILITY,
                ...
                if len(node.getElementsByTagName('intent-filter')) > 0:
                    for x in node.getElementsByTagName('intent-filter'):
                        ...
                        try:
                            ...
                            common.logger.error(
                                "Error de-duplicating command list in adb.py while processing receivers: " + str(e))
                        for c in cmd_list:
                            if len(c) > 0:
                                output = False
                                extras_list = []
