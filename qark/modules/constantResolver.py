                                                    ...
                                                    if j_dots is not None:
                                                        if j_dots[len(j_dots) - 2] == value_dots[0]:
                                                            ...
                                                            try:
                                                                ...
                                                                common.logger.error(
                                                                    "Unable to create tree for import while looking for extras: " + str(
                                                                        e))
                                                                break
                                                            if len(var_found) == 0:
                                                                try:
                                                                        ...
                                                                        if type(type_decl) is m.ClassDeclaration:
                                                                            if len(var_found) == 0:
                                                                                for t in type_decl.body:
                                                                                    var_found = recursive_var_finder(t,
                                                                                                                     value_dots)
                                                                                    if len(var_found) > 0:
                                                                                        return var_found
                                                                except Exception as e:
                                                                    common.logger.error(
                                                                        "Problem in constantResolver.main trying to process new_tree: " + str(
                                                                            e))
                else:
                                                ...
                                                if len(var_found) == 0:
                                                    for t in type_decl.body:
                                                        try:
                                                            ...
                                                            common.logger.error(
                                                                "Problem trying to recursively find variables: " + str(
                                                                    e))
                                                        if len(var_found) > 0:
                                                            return var_found
                                    except Exception as e:
                                        common.logger.error(
                                            "Problem in constantResolver.main, processing second_tree: " + str(e))
        else:
                        ...
                        if found is not None:
                            if len(found) > 0:
                                ...
                                try:
                                    ...
                                    common.logger.error(
                                        "Problem trying to recursively find variables from list: " + str(e))
                        ...
                        if found is not None:
                            if len(found) > 0:
                                ...
                                try:
                                    ...
                                    common.logger.error(
                                        "Problem trying to recursively find variables from fields: " + str(e))
    except Exception as e:
            ...
            if type(type_decl) is m.ClassDeclaration:
                for t in type_decl.body:
                    try:
                        ...
                        common.logger.error(
                            "Problem calling recursive_string_finder from string_finder in constantResolver.py: " + str(
                                e))
    except Exception as e:
