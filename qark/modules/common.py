...
def check_export(tag, output):
                        ...
                        for x in node.getElementsByTagName('intent-filter'):
                            for a in node.getElementsByTagName('action'):
                                if (a.attributes['android:name'].value) not in protected_broadcasts:
                                    exported_perm_list.append([node.attributes['android:name'].value,
                                                               node.attributes['android:permission'].value])
                                else:
                                    protected_broad_list.append(node.attributes['android:name'].value)
                    else:
                        exported_perm_list.append(
                            [node.attributes['android:name'].value, node.attributes['android:permission'].value])
            ...
            for x in node.getElementsByTagName('intent-filter'):
                ...
                if 'android:permission' in node.attributes.keys():
                    if (node.attributes['android:name'].value,
                        node.attributes['android:permission'].value) not in exported_perm_list:
                        if x.getElementsByTagName('action'):
                            for a in node.getElementsByTagName('action'):
                                if (node.attributes['android:name'].value,
                                    a.attributes['android:name'].value) not in protected_broadcasts:
                                    exported_perm_list.append([node.attributes['android:name'].value,
                                                               node.attributes['android:permission'].value])
                                else:
                                    protected_broad_list.append(
                                        [node.attributes['android:name'].value, node.attributes['android:name'].value])
                        else:
                            exported_perm_list.append(
                                [node.attributes['android:name'].value, node.attributes['android:permission'].value])
                    else:
                            ...
                            if x.getElementsByTagName('action'):
                                for a in node.getElementsByTagName('action'):
                                    if (a.attributes['android:name'] not in protected_broadcasts):
                                        ...
                                        protected_broad_list.append(
                                            [node.attributes['android:name'].value, a.attributes['android:name'].value])
                            else:
                                exported_list.append(node.attributes['android:name'].value)
                else:
                    if x.getElementsByTagName('action'):
    ...
    if output:
        ...
        if (int(minSdkVersion) < 20 and len(exported_perm_list) > 1):
            ...
            issue.setData(config.get('qarkhelper', 'WARN_FOLL') + ' ' + tag + ' ' + config.get('qarkhelper',
                                                                                               'PERM_SNATCH') + ' ' + tag + ' ' + config.get(
                'qarkhelper', 'VULN_MAL') + ' ' + tag + ' ' + config.get('qarkhelper', 'VULN_TYPES'))
            issue.setExtras("list", exported_perm_list)
            report_data.append(issue)

            issue = ReportIssue()
            issue.setCategory(ExploitType.MANIFEST)
            issue.setDetails(config.get('qarkhelper', 'WARN_FOLL') + ' ' + tag + ' ' + config.get('qarkhelper',
                                                                                                  'PERM_SNATCH') + ' ' + tag + ' ' + config.get(
                'qarkhelper', 'VULN_MAL') + ' ' + tag + ' ' + config.get('qarkhelper', 'VULN_TYPES'))
        ...
        if len(protected_broad_list) > 1:
            ...
            issue.setLevel(Severity.INFO)
            issue.setData(
                "These are exported, but the associated Intents can only be sent by SYSTEM level apps. They could still potentially be vulnerable, if the Intent carries data that is tainted (2nd order injection)")
            issue.setExtras("list", protected_broad_list)
            report_data.append(issue)
...
def sink_list_check(token, tree):
    ...
    sink_list.append(['java.net.URL', 'void', 'set',
    ...
    sink_list.append(['android.content.Intent', 'android.content.Intent', 'setClassName',
    ...
    sink_list.append(['android.content.Intent', 'android.content.Intent', 'putExtra',
    ...
    sink_list.append(['android.content.Context', 'android.content.Intent', 'registerReceiver',
    ...
    sink_list.append(['android.telephony.SmsManager', 'void', 'sendTextMessage',
                      ['java.lang.String', 'java.lang.String', 'java.lang.String', 'android.app.PendingIntent',
                       'android.app.PendingIntent']])
    sink_list.append(['android.telephony.SmsManager', 'void', 'sendDataMessage',
                      ['java.lang.String', 'java.lang.String', 'short', 'byte[]', 'android.app.PendingIntent',
                       'android.app.PendingIntent']])
    sink_list.append(['android.telephony.SmsManager', 'void', 'sendMultipartTextMessage',
    ...
    sink_list.append(['android.content.SharedPreferences$Editor', 'android.content.SharedPreferences$Editor', 'putInt',
                      ['java.lang.String', 'int']])
    sink_list.append(['android.content.SharedPreferences$Editor', 'android.content.SharedPreferences$Editor', 'putLong',
                      ['java.lang.String', 'long']])
    sink_list.append(
        ['android.content.SharedPreferences$Editor', 'android.content.SharedPreferences$Editor', 'putString',
         ['java.lang.String', 'java.lang.String']])
    sink_list.append(['org.apache.http.impl.client.DefaultHttpClient', 'org.apache.http.HttpResponse', 'execute',
                      ['org.apache.http.client.methods.HttpUriRequest']])
    sink_list.append(['org.apache.http.client.HttpClient', 'org.apache.http.HttpResponse', 'execute',
                      ['org.apache.http.client.methods.HttpUriRequest']])
    ...
    sink_list.append(['android.content.Context', 'boolean', 'bindService',
    ...
    sink_list.append(['android.app.Activity', 'boolean', 'bindService',
                      ['android.content.Intent', 'android.content.ServiceConnection', 'int']])
    ...
    sink_list.append(['android.app.Activity', 'void', 'sendBroadcastAsUser',
                      ['android.content.Intent', 'android.os.UserHandle', 'java.lang.String']])
    sink_list.append(['android.app.Activity', 'void', 'sendOrderedBroadcast',
    ...
    sink_list.append(['android.app.Activity', 'void', 'sendOrderedBroadcastAsUser',
                      ['android.content.Intent', 'android.os.UserHandle', 'java.lang.String',
    ...
    sink_list.append(['android.app.Activity', 'void', 'sendStickyBroadcastAsUser',
                      ['android.content.Intent', 'android.os.UserHandle']])
    sink_list.append(['android.app.Activity', 'void', 'sendStickyOrderedBroadcast',
                      ['android.content.Intent', 'android.content.BroadcastReceiver', 'android.os.Handler', 'int',
                       'java.lang.String', 'android.os.Bundle']])
    sink_list.append(['android.app.Activity', 'void', 'sendStickyOrderedBroadcastAsUser',
                      ['android.content.Intent', 'android.os.UserHandle', 'android.content.BroadcastReceiver',
                       'android.os.Handler', 'int', 'java.lang.String', 'android.os.Bundle']])
    sink_list.append(['android.content.ContentResolver', 'android.net.Uri', 'insert',
                      ['android.net.Uri', 'android.content.ContentValues']])
    sink_list.append(['android.content.ContentResolver', 'int', 'delete',
                      ['android.net.Uri', 'java.lang.String', 'java.lang.String[]']])
    sink_list.append(['android.content.ContentResolver', 'int', 'update',
                      ['android.net.Uri', 'android.content.ContentValues', 'java.lang.String', 'java.lang.String[]']])
    sink_list.append(['android.content.ContentResolver', 'android.database.Cursor', 'query',
    ...
    sink_list.append(['java.net.URL', 'void', '<init>',
        ...
        for s in sink_list:
            if len(s) > 0:
                ...
                if hasattr(token, 'name'):
                    ...
                    logger.error("Unable to find the name of the token: " + str(
                        token) + " when checking against sink list in findMethods.py")
                    found = None
            else:
                found = None
    except Exception as e:
