...
def find_extras(stub_file_name, entry):
    ...
    common.logger.info(
        "Checking for extras in this file: " + str(stub_file_name) + " from this entry point: " + str(entry))
    unmatched = True
    ...
    if unmatched:
        if str(j) not in common.file_not_found:
            ...
            try:
                common.logger.error(
                    "Sorry, we could not find/parse a file named: " + str(j) + " while looking for extras\n")
            except Exception as e:
                                ...
                                if resolved_constant is not None:
                                    if len(resolved_constant) > 0:
                                        if resolved_constant not in extras:
                                            ...
                                            extras.append(var_type)
                                            common.logger.info(
                                                "Possible Extra: " + str(resolved_constant) + " of type: " + str(
                                                    var_type))
                                        if token.name in single_extra:
                                            break
                                    else:
                                            ...
                                            if resolved_constant is not None:
                                                if len(resolved_constant) > 0:
                                                    if resolved_constant not in extras:
                                                        extras.append(resolved_constant)
                                                        extras.append(var_type)
                                                        common.logger.info("Possible Extra: " + str(
                                                            resolved_constant) + " of type: " + str(var_type))
                                                    if token.name in single_extra:
                                                        break
                                                else:
                                                    common.logger.warning("Unable to resolve constant: " + str(
                                                        a.value) + ". You should review this manually.")
                                if resolved_constant is not None:
                                    if len(resolved_constant) == 0:
                                        common.logger.warning("Unable to resolve constant: " + str(
                                            a.value) + ". You should review this manually.")
