...
def default_wv_config(wv, srcfile, sdk_ver, results):
    ...
    issue.setData(common.config.get('qarkhelper', 'FILE_SYS_WARN1') + common.config.get('qarkhelper',
                                                                                        'FILE_SYS_WARN2') + " To validate this vulnerability, load the following local file in this WebView: file://<install_dir>/qark/poc/html/FILE_SYS_WARN.html")
    results.append(issue)

    issue = terminalPrint()
    issue.setLevel(Severity.WARNING)
    issue.setData(common.config.get('qarkhelper',
                                    'WV_CPA_WARNING') + " To validate this vulnerability, load the following local file in this WebView: file://<install_dir>/quark/poc/html/WV_CPA_WARNING.html")
    results.append(issue)
    if sdk_ver < 16:
        ...
        issue.setLevel(Severity.WARNING)
        issue.setData(common.config.get('qarkhelper',
                                        'UNIV_FILE_WARNING') + " To validate this vulnerability, load the following local file in this WebView: file://<install_dir>/quark/poc/html/UNIV_FILE_WARNING.html")
        results.append(issue)
        ...
        if sdk_ver < 16:
            ...
            issue.setLevel(Severity.WARNING)
            issue.setData(common.config.get('qarkhelper',
                                            'FURL_FILE_WARNING') + " To validate this vulnerability, load the following local file url in this WebView: file://<install_dir>/quark/poc/html/FURL_FILE_WARNING.html")
            results.append(issue)
        else:
            issue = terminalPrint()
            issue.setLevel(Severity.INFO)
    ...
    for f in s_list:
        ...
        if wv_config(i[1], wv_js_check):
            ...
            issue.setData(
                common.config.get('qarkhelper', 'TERMINAL_JS_WARNING') + " " + str(i[0]) + " " + common.config.get(
                    'qarkhelper',
                    'TERMINAL_JS_WARNING1') + " To validate this vulnerability, load the following local file in this WebView: file://<install_dir>/quark/poc/html/JS_WARNING.html\n")
            results.append(issue)
        else:
            issue = terminalPrint()
            issue.setLevel(Severity.INFO)
        ...
        if wv_config(i[1], wv_burl_check):
            ...
            issue.setData(
                common.config.get('qarkhelper', 'TERMINAL_BURL_WARNING1') + " " + str(i[0]) + " " + common.config.get(
                    'qarkhelper',
                    'TERMINAL_BURL_WARNING2') + "To validate this vulnerability, load the following local file in this WebView: file://<install_dir>/quark/poc/html/BURL_WARNING.html\n")
            results.append(issue)
        else:
            issue = ReportIssue()
            issue.setCategory(ExploitType.WEBVIEW)
        ...
        if wv_config(i[1], wv_file_check):
            ...
            issue.setData(
                common.config.get('qarkhelper', 'TERMINAL_FILE_SYS_WARN1') + str(i[0]) + " " + common.config.get(
                    'qarkhelper',
                    'TERMINAL_FILE_SYS_WARN2') + " To validate this vulnerability, load the following local file in this WebView: file://<install_dir>/quark/poc/html/FILE_SYS_WARN.html\n")
            results.append(issue)
        ...
        if wv_config(i[1], wv_cpa_check):
            ...
            issue.setData(common.config.get('qarkhelper', 'TERMINAL_WV_CPA_WARNING') + str(i[
                                                                                               0]) + "To validate this vulnerability, load the following local file in this WebView: file://<install_dir>/quark/poc/html/WV_CPA_WARNING.html\n")
            results.append(issue)
        ...
        if int(common.minSdkVersion) < 16:
            ...
            if not wv_config(i[1], wv_univ_file_access):
                ...
                issue.setData(common.config.get('qarkhelper', 'TERMINAL_UNIV_FILE_WARNING') + str(i[
                                                                                                      0]) + " To validate this vulnerability, load the following local file in this WebView: file://<install_dir>/quark/poc/html/UNIV_FILE_WARNING.html\n")
                results.append(issue)
                skip_next = True
            else:
                issue = ReportIssue()
            ...
            if skip_next:
                ...
                if wv_config(i[1], wv_allow_file_access_furls):
                    ...
                    issue.setLevel(Severity.INFO)
                    issue.setData(
                        "This WebView does not have access to File URLs - setAllowFileAccessFromFileURLs(false)" + str(
                            i[0]))
                    results.append(issue)
                    ...
                    issue.setExtras(IS_FILE_ACCESS_ENABLED, True)
                    issue.setData(common.config.get('qarkhelper', 'TERMINAL_UNIV_FILE_WARNING') + str(i[
                                                                                                          0]) + "To validate this vulnerability, load the following local file in this WebView: file://<install_dir>/quark/poc/html/UNIV_FILE_WARNING2.html\n")
                    results.append(issue)
            ...
            if wv_config(i[1], wv_univ_file_access):
                ...
                issue.setData(common.config.get('qarkhelper', 'TERMINAL_UNIV_FILE_WARNING') + '1 ' + str(i[
                                                                                                             0]) + " To validate this vulnerability, load the following local file url in this WebView: file://<install_dir>/quark/poc/html/UNIV_FILE_WARNING.html\n")
                results.append(issue)
                skip_next = True
                ...
                if skip_next:
                    ...
                    if wv_config(i[1], wv_allow_file_access_furls):
                        ...
                        issue.setData(common.config.get('qarkhelper', 'TERMINAL_FURL_FILE_WARNING') + str(i[
                                                                                                              0]) + "To validate this vulnerability, load the following local file in this WebView: file://<install_dir>/quark/poc/html/FURL_FILE_WARNING.html\n")
                        results.append(issue)
                    else:
                        issue = ReportIssue()
                        issue.setCategory(ExploitType.WEBVIEW)
        ...
        if wv_config(i[1], wv_plugsinenabled):
            if int(common.minSdkVersion) < 18:
                ...
                issue.setCategory(ExploitType.WEBVIEW)
                issue.setDetails(
                    common.config.get('qarkhelper', 'DEPRECATED_SINCE_9') + str(i[0]) + "<br>FILE: " + str(i[1]))
                issue.setFile(str(i[1]))
                issue.setSeverity(Severity.INFO)
                results.append(issue)
                ...
                issue.setCategory(ExploitType.WEBVIEW)
                issue.setDetails(
                    common.config.get('qarkhelper', 'REMOVED_IN_18') + str(i[0]) + "<br>FILE: " + str(i[1]))
                issue.setFile(str(i[1]))
                issue.setSeverity(Severity.INFO)
                results.append(issue)
        ...
        if wv_config(i[1], wv_ajs):
            if int(common.minSdkVersion) < 17:
                ...
                issue.setData(common.config.get('qarkhelper', 'TERMINAL_BAD_JS_INT') + " " + str(i[
                                                                                                     0]) + " To validate this vulnerability, load the following local file in this WebView: file://<install_dir>/quark/poc/html/BAD_JS_INT.html\n")
                results.append(issue)
            else:
                issue = ReportIssue()
                issue.setCategory(ExploitType.WEBVIEW)
