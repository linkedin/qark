                                        ...
                                        if str(a.target.value) == str(t):
                                            ...
                                            if first_pass:
                                                common.logger.log(common.VULNERABILITY_LEVEL,
                                                                  "The " + str(
                                                                      token.name) + " method, imported from " + str(
                                                                      imp_decl.name.value) + " is tainted by " + str(
                                                                      a.target.value) + " on object " + token.target.value + " of type " + str(
                                                                      obj_inst))
                                                first_pass = False
                                            ...
                                            common.logger.log(common.VULNERABILITY_LEVEL,
                                                              "To exploit, you will need to send an intent with the key: " + str(
                                                                  extras[1]) + " of type: " + str(
                                                                  re.sub(r'get', '', extras[0])))
                                            issueExtras.append(
                                                "To exploit, you will need to send an intent with the key: " + str(
        ...
        try:
            ...
            if component_type == 'activity':
                try:
                    ...
                    common.logger.error(
                        "Problem calling find_set_result in parse_method_invocation for supers: " + str(e))
        except Exception as e:
        ...
        if token.name not in entries:
            if type(tree) is not None:
                ...
                try:
                    ...
                    common.logger.error(
                        "Problem trying to look for locally declared methods in findMethods.py: " + str(e))
                if not closeEnough:
                    try:
                        ...
                        common.logger.error(
                            'Problem running externalMethodDeclarations module from findMethods.py: ' + str(e))
            else:
            ...
            try:
                ...
                common.logger.error(
                    "Problem in parse_method_invocation of findMethods.py, when trying to match sinks: " + str(e))
    return
